...
class ArcaScrapper:
    def __init__(self, parser: str = "html.parser", workers: Union[int, None] = None):
    ...
    def get_page_contents_start_to_end(
        ...
        def get_post_contents_and_sleep(post_url: str, pbar: tqdm):
            contents = self.get_post_contents(post_url)
            pbar.update()
            time.sleep(0.3)
            return contents
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            ...
            urls = list(chain.from_iterable(urls_iter))
            pbar = tqdm(total=len(urls), desc="post")
